function SCHEMA:PlayerFootstep(client, position, foot, soundName, volume)
	if (client:isRunning()) then
		if (client:Team() == FACTION_TROOPER) then
			client:EmitSound("npc/metropolice/gear"..math.random(1, 6)..".wav", volume * 130)

			return true
		elseif (client:Team() == FACTION_SHADOWTROOPER) then
...
function SCHEMA:OnCharCreated(client, character)
	...
	if (inventory) then		
		if (character:getFaction() == FACTION_REBEL) then
			inventory:add("cid", 1, {
...
function SCHEMA:GetPlayerDeathSound(client)
	if (client:isCombine()) then
		local sounds = self.deathSounds[client:Team()] or self.deathSounds[FACTION_TROOPER]
		...
		self:addDisplay("WARNING Unit lost: "..digits.."", Color(255, 0, 0))
...
function SCHEMA:PlayerTick(client)
	if (client:isCombine() and client:Alive() and (client.nutHealthCheck or 0) < CurTime()) then
		...
		if (client:Health() <= 10) then
			delay = 10
			client:addDisplay("Vital signs falling for nearby unit.", Color(255, 0, 0))
		elseif (client:Health() <= 25) then
			delay = 20
			client:addDisplay("Vital signs CRITICAL for nearby unit, assistaince required.", Color(255, 100, 0))
		elseif (client:Health() <= 50) then
			delay = 45
			client:addDisplay("Local soldier is advised to seek for medical attention if possible.", Color(255, 175, 0))
		end
...
netstream.Hook("obj", function(client, text)
	if (hook.Run("CanPlayerEditObjectives", client)) then
		...
		SCHEMA:addDisplay(client:Name().." has updated the objectives list.", Color(0, 0, 255))
